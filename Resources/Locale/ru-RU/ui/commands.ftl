...
## 'help' command
cmd-oldhelp-desc = Выводит общую справку или справку по определенной команде
cmd-oldhelp-help = Использование: help [имя команды]
...
cmd-oldhelp-no-args = Чтобы получить справку по определенной команде, используйте 'help <command>'. Для получения списка всех доступных команд используйте 'list'. Для поиска по командам используйте 'list <filter>'.
cmd-oldhelp-unknown = Неизвестная команда: { $command }
cmd-oldhelp-top = { $command } - { $description }
cmd-oldhelp-invalid-args = Недопустимое количество аргументов.
cmd-oldhelp-arg-cmdname = [имя команды]
...
# Note: команда flushcookies взята из Robust.Client.WebView, её нет в коде основного движка.
