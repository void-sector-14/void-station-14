...
cmd-mapping-success-load-grid = Загружает неинициализированную сетку из файла { $path } на новую карту с id { $mapId }.
cmd-mapping-success = Создаёт неинициализированную карту с id { $mapId }.
cmd-mapping-warning = ПРЕДУПРЕЖДЕНИЕ: На сервере используется отладочная DEBUG сборка. Вы рискуете потерять свои изменения.
