...
ent-MobGoliath = голиаф
    .desc = Огромный зверь, который использует длинные щупальца, чтобы поймать свою добычу, угрожать им не рекомендуется ни при каких обстоятельствах.
ent-ActionGoliathTentacle = [color=red]Удар Щупальцем[/color]
    .desc = Используй свои щупальца чтобы схватить и оглушить выбранного противника!
...
ent-MobBasilisk = василиск
    .desc = Территориальное чудовище, покрытое толстой оболочкой, поглощающей энергию. Его взгляд заставляет жертву замерзать изнутри.
