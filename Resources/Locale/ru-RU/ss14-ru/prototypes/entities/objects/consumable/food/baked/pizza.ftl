ent-FoodPizzaBase = { ent-FoodInjectableBase }
    .desc = { ent-FoodInjectableBase.desc }
ent-FoodPizzaSliceBase = { ent-FoodInjectableBase }
    .desc = { ent-FoodInjectableBase.desc }
ent-FoodPizzaMargherita = пицца маргарита
    .desc = Самая сырная пицца в галактике.
ent-FoodPizzaMargheritaSlice = кусок пиццы маргарита
    .desc = Кусочек самой сырной пиццы в галактике.
ent-FoodPizzaMeat = мясная пицца
    .desc = Сочная пицца с вкусным мясом.
ent-FoodPizzaMeatSlice = кусок мясной пиццы
    .desc = Питательный кусочек мясной пиццы.
ent-FoodPizzaMushroom = грибная пицца
    .desc = Очень необычная пицца.
ent-FoodPizzaMushroomSlice = кусок грибной пиццы
    .desc = Может быть, это последний кусок пиццы в вашей жизни.
ent-FoodPizzaVegetable = овощная пицца
    .desc = Вегетарианцы станции скажут вам спасибо за это.
ent-FoodPizzaVegetableSlice = кусок овощной пиццы
    .desc = Одного кусочка достаточно, чтобы удовлетворить даже самый придирчивый персонал станции.
ent-FoodPizzaDonkpocket = пицца донк-покет
    .desc = Кто подумал, что это будет хорошая идея?
ent-FoodPizzaDonkpocketSlice = кусок пиццы донк-покет
    .desc = Пахнет как донк-покет
ent-FoodPizzaDank = расслабляющая пицца
    .desc = Любимая пицца хиппи.
ent-FoodPizzaDankSlice = кусок расслабляющей пиццы
    .desc = Так хорошо, чувак...
ent-FoodPizzaSassysage = дерзкалбасная пицца
    .desc = Вы действительно чувствуете запах нахальства.
ent-FoodPizzaSassysageSlice = кусок дерзкалбасной пиццы
    .desc = Восхитительно дерзкая, восхитительно колбасная.
ent-FoodPizzaPineapple = гавайская пицца
    .desc = Заставляет людей рыдать. Слезы ли это радости или же печали, зависит от любви человека к ананасу.
ent-FoodPizzaPineappleSlice = кусок Гавайской пиццы
    .desc = Кусочек радости/греха.
ent-FoodPizzaArnold = пицца Арнольда
    .desc = Здравствуйте, Вы позвонили в пиццерию Арнольда. Меня сейчас нет, я отошёл убивать пепперони.
ent-FoodPizzaArnoldSlice = кусок пиццы Арнольда
    .desc = Я приду и, может быть, угощу тебя пиццей, а может быть, отломаю тебе руку.
ent-FoodPizzaMoldySlice = кусок заплесневелой пиццы
    .desc = Когда-то это был отличный кусок пиццы, но теперь он лежит здесь, прогорклый и пронизанный спорами.
ent-FoodPizzaUranium = острокаменная пицца
    .desc = Острая пицца с перцем и ураном.
ent-FoodPizzaUraniumSlice = кусок острокаменной пиццы
    .desc = Светящийся кусок острокаменной пиццы.
ent-FoodPizzaCotton = хлопковая пицца
    .desc = Плоское тканевое тесто из хлопка с хлопковой начинкой. Невероятно.
ent-FoodPizzaCottonSlice = кусок хлопковой пиццы
    .desc = Кусок хлопковой пиццы. Хлопок включён.
ent-FoodPizzaWorldpeas = мировая горохопицца
    .desc = Современная дипломатия в форме диска.
ent-FoodPizzaWorldpeasSlice = кусок мировой горохопиццы
    .desc = Разделение мира — это небольшая цена за гармонию.
