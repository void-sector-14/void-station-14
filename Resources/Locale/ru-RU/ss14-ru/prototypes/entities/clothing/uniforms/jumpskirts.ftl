ent-ClothingUniformJumpskirtBartender = униформа бармена
    .desc = Красивая и опрятная форма. Жаль только, что про бар этого не скажешь.
    .suffix = { "" }
ent-ClothingUniformJumpskirtCaptain = юбка-комбинезон капитана
    .desc = Синяя юбка-комбинезон с золотыми нашивками, указывающими на звание капитана.
    .suffix = { "" }
ent-ClothingUniformJumpskirtCargo = юбка-комбинезон грузчика
    .desc = Прочная юбка-комбинезон, выдаваемая сотрудникам отдела поставок.
    .suffix = { "" }
ent-ClothingUniformJumpskirtChiefEngineer = юбка-комбинезон старшего инженера
    .desc = Эта юбка-комбинезон повышенной заметности, которая выдается тем инженерам, которые достаточно безумны и живучи, чтобы достичь звания старшего инженера. Имеет слабую защиту от радиации.
    .suffix = { "" }
ent-ClothingUniformJumpskirtChaplain = юбка-комбинезон священника
    .desc = Это чёрная юбка-комбинезон, зачастую носимая религиозными людьми.
    .suffix = { "" }
ent-ClothingUniformJumpskirtChef = униформа шеф-повара
    .desc = Без неё ничего не приготовить.
    .suffix = { "" }
ent-ClothingUniformJumpskirtChemistry = юбка-комбинезон химика
    .desc = На этом комбинезоне какие-то странные пятна. Хм.
    .suffix = { "" }
ent-ClothingUniformJumpskirtVirology = юбка-комбинезон вирусолога
    .desc = Она сделана из специального волокна, которое обеспечивает особую защиту от биологических опасностей. На ней есть нашивка со званием вирусолога.
    .suffix = { "" }
ent-ClothingUniformJumpskirtGenetics = юбка-комбинезон генетика
    .desc = Она сделана из специального волокна, которое обеспечивает особую защиту от биологических опасностей. На ней есть нашивка со званием генетика.
    .suffix = { "" }
ent-ClothingUniformJumpskirtCMO = юбка-комбинезон главного врача
    .desc = Это юбка-комбинезон, носимая теми, у кого есть опыт работы на должности главного врача. Она обеспечивает слабую биологическую защиту.
    .suffix = { "" }
ent-ClothingUniformJumpskirtDetective = поношенный костюм
    .desc = Тот, кто носит это, олицетворяет занятость.
    .suffix = { "" }
ent-ClothingUniformJumpskirtDetectiveGrey = нуарный костюм
    .desc = Крутой серый костюм частного сыщика с зажимом для галстука.
    .suffix = { "" }
ent-ClothingUniformJumpskirtEngineering = инженерная юбка-комбинезон
    .desc = Если бы этот костюм был изолированным, возможно, инженеры действительно выполняли бы свою чертову работу.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHoP = юбка-комбинезон главы персонала
    .desc = Довольно безвкусная и невзрачная. Идеально подходит для того, чтобы исчезнуть с лица Вселенной.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHoS = юбка-комбинезон главы службы безопасности
    .desc = Она ярко-красная и хрустящая, какими часто бывают жертвы службы безопасности. Её прочная ткань обеспечивает слабую защиту от колющих и режущих повреждений.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHoSAlt = водолазка главы службы безопасности
    .desc = Это водолазка, носимая теми, кто достаточно силен и дисциплинирован, чтобы достичь должности главы службы безопасности. Её прочная ткань обеспечивает слабую защиту от колющих и режущих повреждений.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHoSParadeMale = парадная форма главы службы безопасности
    .desc = Роскошная униформа главы службы безопасности, для особых случаев.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHydroponics = юбка-комбинезон ботаника
    .desc = Имеет сильный запах земли. Будем надеяться, что она просто грязная, а не вся в земле.
    .suffix = { "" }
ent-ClothingUniformJumpskirtJanitor = юбка-комбинезон уборщика
    .desc = Юбка-комбинезон для бедняги со шваброй.
    .suffix = { "" }
ent-ClothingUniformJumpskirtMedicalDoctor = юбка-комбинезон врача
    .desc = Она изготовлена из специального волокна, обеспечивающего слабую защиту от биологических опасностей. На груди имеется крест, обозначающий, что носитель является медицинским работником.
    .suffix = { "" }
ent-ClothingUniformJumpskirtMime = юбка-комбинезон мима
    .desc = ...
    .suffix = { "" }
ent-ClothingUniformJumpskirtParamedic = юбка-комбинезон парамедика
    .desc = На нем есть плюс, это хорошо, верно?
    .suffix = { "" }
ent-ClothingUniformJumpskirtBrigmedic = юбка-комбинезон бригмедика
    .desc = Эта форма выдаётся квалифицированному персоналу, прошедшему обучение. Никого не волнует, что обучение заняло полдня.
    .suffix = { "" }
ent-ClothingUniformJumpskirtPrisoner = юбка-комбинезон заключенного
    .desc = Потрачено.
    .suffix = { "" }
ent-ClothingUniformJumpskirtQM = юбка-комбинезон квартирмейстера
    .desc = Что Браун может сделать для Вас?
    .suffix = { "" }
ent-ClothingUniformJumpskirtQMTurtleneck = водолазка квартирмейстера
    .desc = Остромодная водолазка, созданная для тяжёлой работы в отделе Карго.
    .suffix = { "" }
ent-ClothingUniformJumpskirtResearchDirector = водолазка научного руководителя
    .desc = Это водолазка, носимая умными людьми, чтобы достичь должности научного руководителя. Её ткань обеспечивает слабую биологическую защиту.
    .suffix = { "" }
ent-ClothingUniformJumpskirtScientist = юбка-комбинезон учёного
    .desc = Она сделана из специального волокна, которое обеспечивает слабую защиту от взрывов. На ней есть обозначения, указывающие что носитель является учёным.
    .suffix = { "" }
ent-ClothingUniformJumpskirtRoboticist = юбка-комбинезон робототехника
    .desc = Облегающий чёрный костюм с усиленными швами. Отлично подходит для промышленной работы.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSec = юбка-комбинезон службы безопасности
    .desc = Юбка-комбинезон из прочного материала, обеспечивающая надежную защиту.
    .suffix = { "" }
ent-ClothingUniformJumpskirtWarden = униформа смотрителя
    .desc = Формальный костюм службы безопасности для офицеров в комплекте с пряжкой для ремня Nanotrasen.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorGrey = серая юбка-комбинезон
    .desc = Серая юбка-комбинезон, напоминающая вам о старых добрых временах.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorBlack = чёрная юбка-комбинезон
    .desc = Обычная чёрная юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorBlue = синяя юбка-комбинезон
    .desc = Обычная синяя юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorGreen = зелёная юбка-комбинезон
    .desc = Обычная зелёная юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorOrange = оранжевая юбка-комбинезон
    .desc = Не носите её рядом с параноидальными сотрудниками службы безопасности.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorPink = розовая юбка-комбинезон
    .desc = Просто глядя на это, Вы чувствуете себя потрясающе.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorRed = красная юбка-комбинезон
    .desc = Обычная красная юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorWhite = белая юбка-комбинезон
    .desc = Обычная белая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorYellow = жёлтая юбка-комбинезон
    .desc = Обычная жёлтая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorDarkBlue = тёмно-синяя юбка-комбинезон
    .desc = Обычная тёмно-синяя юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorTeal = аквамариновая юбка-комбинезон
    .desc = Обычная аквамариновая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorPurple = фиолетовая юбка-комбинезон
    .desc = Обычная фиолетовая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorDarkGreen = тёмно-зелёная юбка-комбинезон
    .desc = Обычная тёмно-зелёная юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorLightBrown = светло-коричневая юбка-комбинезон
    .desc = Обычная светло-коричневая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorBrown = коричневая юбка-комбинезон
    .desc = Обычная коричневая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtColorMaroon = бордовая юбка-комбинезон
    .desc = Обычная бордовая юбка-комбинезон без знаков различия.
    .suffix = { "" }
ent-ClothingUniformJumpskirtPerformer = юбка-комбинезон артиста
    .desc = Привет, я Скотт, президент Donk Pizza. Вы слышали про [ИЗВЕСТНЫЙ ВИРТУАЛЬНЫЙ ИСПОЛНИТЕЛЬ]?
    .suffix = { "" }
ent-ClothingUniformJumpskirtCapFormalDress = торжественное платье капитана
    .desc = Платье для особых случаев.
    .suffix = { "" }
ent-ClothingUniformJumpskirtCentcomFormalDress = торжественное платье Центком
    .desc = Платье для особых случаев.
    .suffix = { "" }
ent-ClothingUniformJumpskirtHosFormal = торжественное платье главы службы безопасности
    .desc = Платье для особых случаев.
    .suffix = { "" }
ent-ClothingUniformJumpskirtOperative = юбка-комбинезон оперативника
    .desc = Униформа элитных агентов синдиката, выполняющих тактические операции в глубоком космосе..
    .suffix = { "" }
ent-ClothingUniformJumpskirtTacticool = тактикрутая юбка-комбинезон
    .desc = Униформа для оперативных LARPеров, выполняющих тактические кражи изолированных перчаток в глубоком космосе.
    .suffix = { "" }
ent-ClothingUniformJumpskirtAtmos = юбка-комбинезон атмосферного техника
    .desc = Я на работе. Я не могу бросить работу. Работа - это дыхание. Я проверяю качество воздуха.
    .suffix = { "" }
ent-ClothingUniformJumpskirtJanimaid = форма горничной
    .desc = Для профессионалов, а не позёров.
    .suffix = { "" }
ent-ClothingUniformJumpskirtJanimaidmini = форма горничной с мини-юбкой
    .desc = Элитный сервис, а не фантики от конфет.
    .suffix = { "" }
ent-ClothingUniformJumpskirtLawyerRed = красная адвокатская юбка-костюм
    .desc = Яркий красный костюм c юбкой, который носят адвокаты и хвастуны.
    .suffix = { "" }
ent-ClothingUniformJumpskirtLawyerBlue = синяя адвокатская юбка-костюм
    .desc = Яркий синий костюм c юбкой, который носят адвокаты и хвастуны.
    .suffix = { "" }
ent-ClothingUniformJumpskirtLawyerBlack = чёрная адвокатская юбка-костюм
    .desc = Изысканный черный костюм c юбкой, который носят адвокаты и гангстеры.
    .suffix = { "" }
ent-ClothingUniformJumpskirtLawyerPurple = фиолетовая адвокатская юбка-костюм
    .desc = Стильный фиолетовый костюм c юбкой, который носят адвокаты и шоумены.
    .suffix = { "" }
ent-ClothingUniformJumpskirtLawyerGood = хорошая адвокатская юбка-костюм
    .desc = Кричащий костюм c юбкой, идеально подходящий УГОЛОВНОМУ адвокату!
    .suffix = { "" }
ent-ClothingUniformJumpskirtSyndieFormalDress = формальное платье синдиката
    .desc = Форма синдиката, изготовленная в элегантном стиле, в ней даже жалко проделывать грязные трюки.
    .suffix = { "" }
ent-ClothingUniformJumpskirtTacticalMaid = тактическая юбка горничной
    .desc = Считается, что лучшие горничные должны иметь дизайнерские костюмы.
    .suffix = { "" }
ent-ClothingUniformJumpskirtOfLife = юбка жизни
    .desc = Юбка, которая символизирует радость и позитивность нашей жизни.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSeniorEngineer = юбка инженера-инструктора
    .desc = Показатель мастерства и престижа в инженерном отделе.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSeniorResearcher = юбка учёного-инструктора
    .desc = Показатель мастерства и престижа в научном отделе.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSeniorPhysician = юбка врача-инструктора
    .desc = Показатель мастерства и престижа в медицинском отделе.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSeniorOfficer = юбка офицера-инструктора
    .desc = Показатель мастерства и престижа в отделе службы безопасности.
    .suffix = { "" }
ent-ClothingUniformJumpskirtSecGrey = серый комбинезон охраны
    .desc = Тактический пережиток прошлых лет, когда Nanotrasen решила, что дешевле красить костюмы в красный цвет вместо того, чтобы смывать кровь.
    .suffix = { "" }
