# General
ai-wire-snipped = Был перерезан провод, координаты { $coords }.
wire-name-ai-vision-light = ИИВ
wire-name-ai-act-light = ИИС
station-ai-takeover = ИИ захват
# Radial actions
ai-open = Открыть действия
ai-close = Закрыть действия
bolt-close = Заболтировать
bolt-open = Разболтировать
emergency-access-on = Включить аварийный доступ
emergency-access-off = Выключить аварийный доступ
electrify-door-on = Поднять напряжение
electrify-door-off = Снизить напряжение
toggle-light = Переключить свет
ai-device-not-responding = Устройство не отвечает
ai-consciousness-download-warning = Ваше сознание загружается.
# UI
station-ai-customization-menu = Настройка ИИ
station-ai-customization-categories = Категории
station-ai-customization-options = Опции (выбор одной)
station-ai-customization-core = Отображение ядра ИИ
station-ai-customization-hologram = Голографические аватары
# Кастомизация
station-ai-icon-ai = Призрак в машине
station-ai-icon-angel = Ангел-хранитель
station-ai-icon-bliss = Простые времена
station-ai-icon-clown = Клоунское настроение
station-ai-icon-dorf = Приключения ждут
station-ai-icon-heartline = Спасительная линия
station-ai-icon-smiley = Все улыбаются
station-ai-icon-president = Президент
station-ai-icon-catamari = Катамари
station-ai-icon-darkmatter = Тёмная материя
station-ai-icon-database = База данных
station-ai-icon-gentoo = Генту
station-ai-icon-helios = Гелиос
station-ai-icon-matrix = Матрица
station-ai-icon-techdemon = Техно-демон

station-ai-hologram-female = Женский облик
station-ai-hologram-male = Мужской облик
station-ai-hologram-face = Отделённая голова
station-ai-hologram-cat = Кошачья форма
station-ai-hologram-dog = Форма корги
station-ai-hologram-ball = Форма мяча
station-ai-hologram-auto = АВТО
