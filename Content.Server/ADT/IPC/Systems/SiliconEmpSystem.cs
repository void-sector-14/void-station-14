...
public sealed class SiliconEmpSystem : EntitySystem
{
    ...
    private void OnEmpPulse(EntityUid uid, SiliconComponent component, ref EmpPulseEvent args)
    {
        ...
        {
            _stun.TryParalyze(uid, TimeSpan.FromSeconds(Math.Min(duration.TotalSeconds * 0.25f, 15f)), true);
        }

        _status.TryAddStatusEffect<SlowedDownComponent>(uid, "SlowedDown", TimeSpan.FromSeconds(duration.TotalSeconds), false);
